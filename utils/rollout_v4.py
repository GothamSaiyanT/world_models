...
from training.v4_loss import per_sample_drift_error


class RolloutGeneratorV4:
    """Shared V4 rollout logic for baseline, fixed-interval and adaptive."""
    ...
    def __init__(
        ...
        *,
        ...
        if strategy not in {"baseline", "fixed_interval", "adaptive"}:
            raise ValueError("Unknown V4 rollout strategy.")
        self.model = model
    ...
    def generate(self, initial_frame, actions, real_sequence=None):
        ...
        if correction_masks:
            ...
            self.correction_steps = [
                index + 1
                for index, corrected in enumerate(masks)
                if bool(corrected)
            ]
        else:
            self.correction_steps = []
