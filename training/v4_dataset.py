...
class V4SequenceDataset(Dataset):
    """Contiguous 64x64 sequences with explicit train/validation ranges."""

    def __init__(
        self,
        folder: str = "data_v4_64",
        ...
        if frames.ndim != 3 or tuple(frames.shape[1:]) != (64, 64):
            raise ValueError(
                f"V4 expects frames shaped (N,64,64); got {frames.shape}."
            )
        if actions.ndim != 1 or len(actions) != len(frames):
            raise ValueError("V4 actions must be shaped (N,) and match frames.")
...
def dataset_split_points(folder: str = "data_v4_64", train_fraction: float = 0.8):
