from __future__ import annotations

from dataclasses import dataclass

import torch
import torch.nn.functional as F


@dataclass
class V3LossParts:
    total: torch.Tensor
    base: torch.Tensor
    motion: torch.Tensor
    foreground: torch.Tensor
    delta: torch.Tensor
    edge: torch.Tensor
    motion_fraction: torch.Tensor


def _masked_mean(values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    denom = mask.sum().clamp_min(1.0)
    return (values * mask).sum() / denom


def _edge_map(x: torch.Tensor) -> torch.Tensor:
    dx = x[..., :, 1:] - x[..., :, :-1]
    dy = x[..., 1:, :] - x[..., :-1, :]
    return dx, dy


def motion_aware_loss(
    prediction: torch.Tensor,
    real_current: torch.Tensor,
    real_next: torch.Tensor,
    *,
    motion_threshold: float = 0.01,
    foreground_threshold: float = 0.03,
    motion_weight: float = 4.0,
    foreground_weight: float = 1.0,
    delta_weight: float = 3.0,
    edge_weight: float = 0.25,
    dilation: int = 5,
) -> V3LossParts:
    """
    Loss where sparse moving pixels get their own normalized term.

    This avoids the old failure mode where >99% static/background pixels could
    dominate MSE and reward a frozen-frame solution.
    """
    squared = (prediction - real_next).pow(2)
    base = squared.mean()

    raw_motion = (
        (real_next - real_current).abs() > motion_threshold
    ).to(prediction.dtype)
    if dilation > 1:
        raw_motion = F.max_pool2d(
            raw_motion,
            kernel_size=dilation,
            stride=1,
            padding=dilation // 2,
        )
    motion = _masked_mean(squared, raw_motion)

    foreground_mask = (
        real_next.abs() > foreground_threshold
    ).to(prediction.dtype)
    foreground = _masked_mean(squared, foreground_mask)

    target_delta = real_next - real_current
    predicted_delta = prediction - real_current
    delta_squared = (predicted_delta - target_delta).pow(2)
    delta = _masked_mean(delta_squared, raw_motion)

    pred_dx, pred_dy = _edge_map(prediction)
    real_dx, real_dy = _edge_map(real_next)
    edge = (
        F.l1_loss(pred_dx, real_dx)
        + F.l1_loss(pred_dy, real_dy)
    ) * 0.5

    total = (
        base
        + motion_weight * motion
        + foreground_weight * foreground
        + delta_weight * delta
        + edge_weight * edge
    )

    return V3LossParts(
        total=total,
        base=base.detach(),
        motion=motion.detach(),
        foreground=foreground.detach(),
        delta=delta.detach(),
        edge=edge.detach(),
        motion_fraction=raw_motion.mean().detach(),
    )


def per_sample_drift_error(
    prediction: torch.Tensor,
    real_current: torch.Tensor,
    real_next: torch.Tensor,
    motion_threshold: float = 0.01,
) -> torch.Tensor:
    """Per-sample drift score that gives moving pixels meaningful influence."""
    base = (prediction - real_next).pow(2).flatten(1).mean(1)
    mask = ((real_next - real_current).abs() > motion_threshold).to(prediction.dtype)
    mask = F.max_pool2d(mask, 5, stride=1, padding=2)
    squared = (prediction - real_next).pow(2)
    motion_sum = (squared * mask).flatten(1).sum(1)
    motion_count = mask.flatten(1).sum(1).clamp_min(1.0)
    motion = motion_sum / motion_count
    return base + 2.0 * motion
