...
CHECKPOINTS = {
    "baseline": "models_128/best_world_model.npz",
    "adaptive": "models_128/best_adaptive_model.pt",
    "fixed_interval": "models_128/best_fixed_interval_model.pt",
}
    ...
    if pipeline == "baseline":
        ...
        if (
            ...
            if "model_config" in checkpoint_data:
                saved_model_config = checkpoint_data["model_config"]
            elif isinstance(checkpoint_data.get("configs"), dict):
                saved_model_config = checkpoint_data["configs"].get("model")

            if saved_model_config is not None:
                saved_config = ModelConfig(**saved_model_config)
...
def load_dataset():
    return WorldModelDataset(folder="data_128")
...
st.title("World Model Pipeline Comparison — 128×128")
