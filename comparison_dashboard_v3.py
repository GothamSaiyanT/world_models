...
CHECKPOINTS = {
    "baseline": "models_v3/best_baseline_model.pt",
    "adaptive": "models_v3/best_adaptive_model.pt",
    "fixed_interval": "models_v3/best_fixed_interval_model.pt",
}
...
def load_model(pipeline, checkpoint, device):
    ...
    checkpoint_meta = os.path.join(
        os.path.dirname(checkpoint),
        f"best_{pipeline}_checkpoint.pt",
    )
    metadata = {}
    if os.path.exists(checkpoint_meta):
        metadata = torch.load(checkpoint_meta, map_location="cpu")
    model._v3_metadata = metadata
    model.to(device)
    model.eval()
    return model
...
def load_dataset():
    return WorldModelDataset(folder="data_v3")
...
def generate_prediction(
    pipeline, model, initial_frame, actions, real_sequence
):
    metadata = getattr(model, "_v3_metadata", {})
    interval = int(metadata.get("fixed_interval", 8))
    ...
    predicted = rollout.generate(
        ...
        actions=actions,
        real_sequence=(
            real_sequence if pipeline != "baseline" else None
        ),
    )
...
st.title("World Model Pipeline Comparison — V3 Motion-Fixed 128×128")
st.success("Dashboard loaded successfully. Configure the evaluation in the sidebar.")
st.caption(
    "V3 synchronized comparison using one shared architecture. Adaptive and Fixed-Interval are observation-corrected; Baseline is open-loop. "
...
try:
    ...
    st.error(f"Could not load dataset from data_v3/: {exc}")
    st.stop()
